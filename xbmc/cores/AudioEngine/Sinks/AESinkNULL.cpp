...
// raw conventions of CActiveAEBufferPool, one packet per call
constexpr unsigned int RAW_FRAME_SIZE = 1;
constexpr unsigned int RAW_FRAMES = 61440;
} // namespace
...
bool CAESinkNULL::Initialize(AEAudioFormat& format, std::string& device)
{
  ...
  {
    m_format.m_frameSize = RAW_FRAME_SIZE;
    m_format.m_frames = RAW_FRAMES;
  }
  else
  {
...
std::chrono::duration<double> CAESinkNULL::DurationOf(const unsigned int frames) const
{
  if (m_format.m_dataFormat == AE_FMT_RAW)
  {
    return std::chrono::duration<double>(m_format.m_streamInfo.GetDuration() / 1000.0);
  }

  return std::chrono::duration<double>(static_cast<double>(frames) / m_format.m_sampleRate);
}
...
void CAESinkNULL::Consume(const std::chrono::duration<double> duration)
{
  UpdateBufferLevel();
...
{
  Consume(DurationOf(frames));
  return frames;
}

void CAESinkNULL::AddPause(const unsigned int millis)
{
  Consume(std::chrono::milliseconds(millis));
}
